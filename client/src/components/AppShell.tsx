...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="glass-page page-enter min-h-screen text-slate-100">
      ...
      <div className="relative flex min-h-screen">
        <aside className={`${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"} sidebar-glass fixed inset-y-0 left-0 z-30 flex w-72 flex-col border-r border-white/[.07] px-5 py-6 transition-transform lg:static`}>
        ...
        <main className="min-w-0 flex-1"><header className="topbar-glass sticky top-0 z-20 flex h-20 items-center justify-between border-b px-5 sm:px-8 lg:px-10"><button className="mr-3 text-slate-400 lg:hidden" onClick={() => setOpen(true)} aria-label="Open menu"><Menu size={21} /></button><div className="flex items-center gap-2 text-xs uppercase tracking-[.2em] text-slate-500"><span className="h-1.5 w-1.5 rounded-full bg-blue-300" />Personal library</div><div className="flex items-center gap-3"><span className="hidden text-xs text-slate-500 sm:inline">{user?.email}</span><div className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-indigo-300 to-blue-700 text-xs font-semibold">{(user?.name ?? "R").slice(0, 2).toUpperCase()}</div></div></header><div className="mx-auto max-w-[1440px] px-5 py-8 sm:px-8 lg:px-10">{children}</div></main>
      </div>
    </div>
  );
}
